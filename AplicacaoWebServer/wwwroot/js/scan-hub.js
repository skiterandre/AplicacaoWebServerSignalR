(function () {
    ...
    var connection = new signalR.HubConnectionBuilder().withUrl("/sendMessageHub").build();

    connection.on("ReceiveImage", function (imagemDto) {
        var displayedMessage = imagemDto.sender + "'s" + " message: " + imagemDto.message;
        var imagemReceived = document.getElementById("imageReceived");
        var imagemrecebida = 'data:image/jpeg;base64,' + imagemDto.imagem;
        imagemReceived.src = imagemrecebida;
    ...
    document.getElementById("sendMessageBtn").addEventListener("click", function (event) {
        ...
        var image = document.getElementById("imagemUpload").src;
        
        image = image.replace('data:image/jpeg;base64,', '');
        var imageSend = { sender: user, message: message, imagem: image };

        connection.invoke("SendMessage", imageSend).catch(function (err) {
            return console.error(err.toString());
        });
